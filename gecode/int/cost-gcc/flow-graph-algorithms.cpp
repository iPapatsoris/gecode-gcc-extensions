...
private:
													 ...
													 NormalEdge edge, bool onPath)
	{
		...
		if (edge.flow < edge.upperBound)
		{
			...
			if (!onPath)
			{ // should be reduced for the violated edges not on path, but what about
				// updating from advisor? those get reduced or normal cost?
			...
			for (auto node : *updatedNodesOld)
			{
				for (auto &edge : graph.nodeList[node].residualEdgeList)
				{
					if ((dest == NULL ||
							...
							dist[node] != INF_UINT && dist[node] + edge.cost < dist[edge.destNode])
					{
	...
	/*void bellmanFordRandom(unsigned int source, 
			...
			// graph.orderGraph.putAtFront(source);

			unordered_set<unsigned int> updatedNodes1 = {source};
			unordered_set<unsigned int> updatedNodes2;
			unordered_set<unsigned int>* updatedNodesOld = &updatedNodes1;
			unordered_set<unsigned int>* updatedNodesNew = &updatedNodes2;

			//graph.printResidual();
			//cout << "Order graph before processing for source " << source << endl;
			//graph.orderGraph.print();
			// graph.orderGraph.putAtFront(source);
			graph.orderGraph.reset(graph.nodeList, source);
			//cout << "Order graph after processing for source " << source << endl;
			//graph.orderGraph.print();

			auto relax = [](unsigned int* dest, unsigned int node, 
										  unsigned int source, ResidualEdge* edge, 
											vector<int>& dist, vector<unsigned int>& prev, 
											bool& foundUpdate, 
											unordered_set<unsigned int>* updatedNodes) {
				//cout << dist[node] << endl;
				//cout << edge->cost << endl;
				//cout << dist[edge->destNode] << endl;
				if ((dest == NULL || !(node == source && edge->destNode == *dest)) &&
						dist[node] != INF_INT && dist[node] + edge->cost < dist[edge->destNode]) 
				{
					// Ignore direct source->dest edge when looking for shortest path
					// to specific requested destination
					dist[edge->destNode] = dist[node] + edge->cost;
					prev[edge->destNode] = node;
					foundUpdate = true;
					updatedNodes->insert(edge->destNode);
					countt++;
				}
			}; 
			...
			while (!updatedNodesOld->empty()) {
				bool foundUpdate = false;
				for (unsigned int i = 0; i < graph.orderGraph.labelToNode.size(); i++) {
					auto node = graph.orderGraph.labelToNode[i];
					if (node == source) {
						node = graph.orderGraph.labelToNode[0];
					}
					if (!i) {
						node = source;
					}
					if (updatedNodesOld->find(node) != updatedNodesOld->end() || updatedNodesNew->find(node) != updatedNodesNew->end()) {
						if (node == source) {
							for (auto& edge: graph.nodeList[node].residualEdgeList) {
								//cout << "1 Relax for edge " << node << "->" << edge.destNode << endl;
								relax(dest, node, source, &edge, dist, prev, foundUpdate, updatedNodesNew);
							}
						} else {
							for (auto e: graph.orderGraph.lowToHighEdges[node]) {
								auto edge = graph.getResidualEdge(node, e);
								//graph.printResidual();
								//cout << "2 Relax for edge " << node << "->" << e << endl;
								relax(dest, node, source, edge, dist, prev, foundUpdate, updatedNodesNew);
							}
						}
					}
				}
				for (int r = graph.orderGraph.labelToNode.size() - 1; r >= 0; r--) {
					auto node = graph.orderGraph.labelToNode[r];
					if (node == source) {
						node = graph.orderGraph.labelToNode[0];
					}
					if (!r) {
					...
					if (updatedNodesOld->find(node) != updatedNodesOld->end() || updatedNodesNew->find(node) != updatedNodesNew->end()) {
						for (auto e: graph.orderGraph.highToLowEdges[node]) {
							auto edge = graph.getResidualEdge(node, e);
							//cout << "3 Relax for edge " << node << "->" << e << endl;
							relax(dest, node, source, edge, dist, prev, foundUpdate, updatedNodesNew);
						}
					}
				}
			...
			//graph.orderGraph.restoreInitialOrder(source);
			//cout << "Restored order graph" << endl;
			//graph.orderGraph.print(); 
			cout << "Total bellman: " << countt << "\n";
		}*/
	...
	bool minCostFlowIteration(pair<unsigned int, unsigned int> violation)
	{
		...
		unordered_set<unsigned int> visitedNodes;
		int pathCost;
		...
		{ // can opt to not iterate all
			for (auto &e : graph.nodeList[i].residualEdgeList)
			{
				// SIGOURA cost edw kai OXI reduced. alliws sto paper bgianoun arnitika
				if (!e.isBackwards)
				{
					e.reducedCost = e.cost - graph.nodeList[i].potential + graph.nodeList[e.destNode].potential;
				}
				assert(e.reducedCost > 0);
			}
		}
	...
	void findShortestPathsNegativeCosts(unsigned int source,
																			vector<unsigned int> &distances) const
	{
		vector<unsigned int> prev;
		bellmanFordShortestPaths(source, prev, distances, NULL);
	}
	...
	bool findShortestPathNegativeCosts(unsigned int source, unsigned int dest,
																		 ...
																		 unordered_set<unsigned int> &visited, int &cost)
			const
	{
		vector<unsigned int> prev;
																		 ...
																		 unsigned int costLowerBound) const
	{
		...
		while (!heap.empty())
		{
			...
			for (auto &edge : graph.nodeList[node].residualEdgeList)
			{
				...
				unsigned int newDist = dist[node] + edge.reducedCostGAC;
	...
	void disjjtra2(unsigned int source, const unsigned int *const dest, vector<unsigned int> &prev,
								 vector<unsigned int> &dist, unordered_set<unsigned int> &visited) const
	{
		dist.assign(graph.nodeList.size(), INF_UINT);
		prev.assign(graph.nodeList.size(), NONE_UINT);
		dist[source] = 0;
		...
		while (!heap.empty())
		{
			...
			for (auto &edge : graph.nodeList[node].residualEdgeList)
			{
				if (visited.find(edge.destNode) != visited.end() ||
						(dest != NULL && node == source && edge.destNode == *dest) || dist[node] == INF_UINT)
				{
					continue;
				}
									...
									unsigned int m) const
	{
		...
		auto mFactor = [](unsigned int b, unsigned int y, FlowGraph &graph)
		{
			...
			for (unsigned int z = 0; z < graph.totalVarNodes; z++)
			{
				...
				{
					min = std::min(min, edgeZB->reducedCostGAC + edgeZC.reducedCostGAC);
				}
			}
			return min;
		};
		...
		if (edgeSA->flow < edgeSA->upperBound && edgeSB->flow > edgeSB->lowerBound && edgeAY->reducedCostGAC > m - edgeYB->reducedCostGAC)
		{
			cout << "\tCONDITION 1 EARLY PRUNNING " << a << " from " << y << endl;
			return true;
		}
		int mB = mFactor(b, y, graph);
		if (edgeSA->flow < edgeSA->upperBound && edgeSB->flow == edgeSB->lowerBound && edgeAY->reducedCostGAC + mB > m)
		{
			cout << "\tCONDITION 2 EARLY PRUNNING " << a << " from " << y << endl;
			return true;
		}
		int mA = mFactor(a, y, graph);
		if (edgeSA->flow == edgeSA->upperBound && edgeSB->flow > edgeSB->lowerBound && edgeAY->reducedCostGAC + mA > m - edgeYB->reducedCostGAC)
		{
			cout << "\tCONDITION 3 EARLY PRUNNING " << a << " from " << y << endl;
			return true;
		}
		if (edgeSA->flow == edgeSA->upperBound && edgeSB->flow == edgeSB->lowerBound && edgeAY->reducedCostGAC + mA + mB > m)
		{
...
public:
	...
	void benchmark() const
	{
		...
		for (unsigned int it = 0; it < 100000; it++)
		{
			...
			for (unsigned int i = 0; i < graph.nodeList.size(); i++)
			{
				//disjjtra2(i, NULL, prev, dist, visitedNodes);
				bellmanFordShortestPaths(i, prev, dist, NULL);
			}
		}
		return;
	}
	...
	bool updateMinCostFlow(vector<FullEdge> &updatedEdges)
	{
		...
		{
			//				cout << "Advising " << edge.first << "->" << edge.second.destNode << endl;
			updateResidualGraph(edge.first, edge.second.destNode, edge.second, false);
		}
		if (graph.printDebug)
		{
		...
		for (auto &edge : graph.nodeList[graph.sNode()].edgeList)
		{
			if (edge.flow > 0)
			{
				...
				for (auto &edgeBY : graph.nodeList[b].edgeList)
				{
					if (edgeBY.flow == 1)
					{
						...
						for (IntVarValues v(vars[y]); v(); ++v)
						{
							...
							if (a != b)
							{
								if (earlyPrune(a, b, y, graph.costUpperBound - graph.flowCost))
								{
				...
				for (const auto &ay : ayList)
				{
					...
					unsigned int costAY = residualEdge->reducedCostGAC;
					unsigned int costYB = graph.getResidualEdge(y, b)->reducedCostGAC;
					if (reducedDistances[a] > graph.costUpperBound - graph.flowCost - costAY - costYB)
					{
